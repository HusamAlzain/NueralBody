...
import { Box, Button, CssBaseline, MenuItem, TextField, Typography } from '@mui/material';
...
export default function SignIn() { 
  ...
  const {createUser,takeWh} = UserAuth();
  ...
  const {name,password,email,birthdate,gender,weight,height}=user
    try{
      await createUser(user.email,user.password)
      await takeWh(user.name,user.weight,user.height)
      navigate('/Home')
    }catch(event){
...
//--------------------------------    
    const adduser = doc(db,"users" , user.name);
    setDoc(adduser,{
      name: user.name,
      email: user.email,
      password: user.password,
